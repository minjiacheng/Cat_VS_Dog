...
def transparency(img, mask):
  ...
  imgrows, imgcols, imgdepth = img.shape
  
  maskrows, maskcols, maskdepth = mask.shape
  imgresize = cv2.resize(img, (maskcols, maskrows)).astype(np.uint8)
  mask = mask.astype(np.uint8)
  transparent_mask = cv2.addWeighted(imgresize, alpha, mask, beta, gamma, dtype=-1)
  
  transparent_mask = cv2.resize(transparent_mask, (imgcols, imgrows)).astype(np.uint8)
  mask = cv2.resize(mask, (imgcols, imgrows)).astype(np.uint8)
  
  return img, mask, transparent_mask
